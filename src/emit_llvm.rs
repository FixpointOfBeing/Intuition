...
//     let param_tys = vec![];
//     let program_fn_ty =
//         types.func_type(ret_ty.clone(), param_tys);
...
//         entry.term = Terminator::Br{ dest: first.name.clone() };
//     }
//     blocks.push(entry);
...
//     match atom {
//         CAtom::Unit => {
//             let unit_ty = types.struct_of(vec![] );
...
//         num_elements: Some(Operand::ConstantOperand(ConstantRef::new(
...
//     let is_float = matches!(left_ty.as_ref(), InstType::FPType(_));
...
//     let is_float = matches!(op_ty.as_ref(), InstType::FPType(_));
...
// fn flatten_arrow(arrow_ty: &InstType, types: &Types) -> TypeRef {
//     match arrow_ty {
//         InstType::Arrow(param_ty, ret_ty) => {
